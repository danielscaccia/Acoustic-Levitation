...
Patm = 101250; % Atmospheric pressure [Pa] ok
mp = 9; % Disk mass [kg] ok 
g = 9.81; % Gravity [m/s^2] ok 
dp = 0.02; % Disk diameter [m] ok 
Ap = pi * ( dp/2 )^2; % Disk area [m^2] ok
mu = 17.72e-6; % Dynamic Viscosity [N*s/m^2] ok

Cp = 1004.8; % Specific heat [J/kg*K] ok
R = 287; % Gas constant [J/kg*K] ok
mu_oil = 2.9e-1; % Oil viscosity [N*s/m^2] ok
e = 0.8e-6; % Oil gap [m] ok
hp = 2e-3; % Disk thickness [m] ok
smax = 0.0092; % Maximum amplitude of the surface [m]
...
figure
plot(m(1,:),m(3,:))
title('Gas Specific Mass Variation');
xlabel('Time [s]');
ylabel('\rho [kg/m^3]');
grid minor
